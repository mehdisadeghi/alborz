...
func registerRoutes(p *plugin) {
	...
	POST("/tasks/:path/color", p.colorTask)
	POST("/calendar/:path/color", p.colorEvent)
}
...
// among them.
func feedObject(ctx *alborz.Context, address, uid string) (*CalendarInfo, *ical.Calendar, error) {
	settings, err := loadSettings(ctx.Session.Store())
	if err != nil {
		return nil, nil, err
	}
...
func (p *plugin) rawObject(ctx *alborz.Context) error {
	...
	if strings.Contains(path, "://") {
		// An event of a feed has no object of its own on any server; it
		// is handed over as the calendar the page shows.
		...
		body, name = io.NopCloser(&buf), uid+".ics"
	} else {
		c, _, err := p.clientWithCalendars(ctx.Request().Context(), ctx.Session)
		if err != nil {
			return err
		}
		if body, err = c.Open(ctx.Request().Context(), path); err != nil {
			return err
		}
	}
	defer body.Close()
...
func (p *plugin) taskList(ctx *alborz.Context) (TaskList, error) {
	...
	if sortKey != "status" && sortKey != "priority" && sortKey != "summary" && sortKey != "account" && sortKey != "calendar" && sortKey != "due" && sortKey != "added" && sortKey != alborzbase.ViewStarred {
		return TaskList{}, echo.NewHTTPError(http.StatusBadRequest, "invalid task sort")
	}
	if sortDir != "" && sortDir != "asc" && sortDir != "desc" {
		return TaskList{}, echo.NewHTTPError(http.StatusBadRequest, "invalid task sort direction")
	}
	...
	for _, result := range dav.Each(ctx.Request().Context(), sites, func(ctx context.Context, site querySite) ([]caldav.CalendarObject, error) {
		...
		if result.Err != nil {
			return TaskList{}, fmt.Errorf("failed to query tasks from %s: %v", result.Site.cal.Name, result.Err)
		}
		...
		for _, task := range result.Value {
			...
			rows = append(rows, taskRow(&task, result.Site.cal, loc, params))
		}
	}
	...
	value := func(row TaskRow) string {
		...
		case alborzbase.ViewStarred:
			// Marked tasks first, as the mail list orders by its flag.
			if row.Star != "" {
				return "0"
			}
			return "1"
	...
	sort.SliceStable(rows, func(i, j int) bool {
		...
		if a == b {
			a = strings.ToLower(rows[i].Task.Account + "\x00" + rows[i].Calendar.Name + "\x00" + rows[i].Summary)
			b = strings.ToLower(rows[j].Task.Account + "\x00" + rows[j].Calendar.Name + "\x00" + rows[j].Summary)
		}
		if sortDir == "desc" {
			return a > b
		}
...
func (p *plugin) colorTask(ctx *alborz.Context) error {
	return p.colorObject(ctx, func(cal *ical.Calendar) *ical.Component { return getFirstTodo(cal) }, "/tasks")
}
func (p *plugin) colorEvent(ctx *alborz.Context) error {
	return p.colorObject(ctx, func(cal *ical.Calendar) *ical.Component {
		if evs := cal.Events(); len(evs) > 0 {
			return evs[0].Component
		}
		return nil
...
// ObjectStarRenderData is one event's or task's star, which is all that
// changes when a row is marked.
type ObjectStarRenderData struct {
	alborz.BaseRenderData
	Action  string
	Current string
	Next    string
...
// colorObject marks one event or task, from its own page or from its
// row: the star the mail and contact pages have, kept as COLOR (RFC
// 7986 5.9) on the component itself. A refused write leaves the star
// as it was: the answer is the server's state, never the click's.
func (p *plugin) colorObject(ctx *alborz.Context, comp func(*ical.Calendar) *ical.Component, base string) error {
	objPath, err := dav.ParseObjectPath(ctx.Param("path"))
	if err != nil {
		return err
	}
	name := ctx.FormValue("color")
	if !validStarView(name) || name == alborzbase.ViewStarred {
	...
	target := comp(co.Data)
	if target == nil {
		return fmt.Errorf("no component to mark")
	}
	...
	next := ctx.NextOr(ctx.AccountPath(base))
	if !ctx.Partial() {
	...
	return ctx.Render(http.StatusOK, "card-star", &ObjectStarRenderData{
		BaseRenderData: *alborz.NewBaseRenderData(ctx),
		Action:         ctx.AccountPath(base + "/" + url.PathEscape(objPath) + "/color"),
		Current:        name,
		Next:           ctx.FormValue("next"),
		Label:          label,
	})
...
func (p *plugin) completeTask(ctx *alborz.Context) error {
	...
	if ctx.Partial() {
		...
		params := dav.ListParamsIn(target, "account", "cal", "query", "view", "sort", "dir")
...
func taskRow(task *caldav.CalendarObject, cal CalendarInfo, loc *time.Location, params url.Values) TaskRow {
	todo := getFirstTodo(task.Data)
	summary, _ := todo.Props.Text("SUMMARY")
	status, _ := todo.Props.Text("STATUS")
...
func (p *plugin) warm(ctx *alborz.Context, s *alborz.Session) {
	...
	go func() {
		...
		defer cancel()
		if err := p.warmAccount(bg, s, loc); err != nil {
			log.Printf("warm %s calendars: %v", s.Username(), err)
		}
	}()
}
