...
func TestAWriteRefusedAsStaleDropsWhatIsHeld(t *testing.T) {
	srv := &server{calls: map[string]int{}, ctag: "1"}
	c := New()
	rt := c.Transport("u", srv)
	send := func(method, target string) {
		t.Helper()
		req, _ := http.NewRequest(method, "https://dav.example"+target, strings.NewReader("<q/>"))
		req.Header.Set("Depth", "1")
		resp, err := rt.RoundTrip(req)
		if err != nil {
			t.Fatal(err)
	...
	send("REPORT", "/c/cal/")
	srv.missing = true
	send(http.MethodPut, "/c/cal/gone.ics")
	srv.missing = false
	send("REPORT", "/c/cal/")
	if n := srv.count("REPORT"); n != 2 {
		t.Errorf("the list held before a write answered 404 was served again: %d REPORTs", n)
	}
}
