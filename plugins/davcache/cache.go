...
func (t *transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if !cacheable(req.Method) {
		...
		// The object is gone, or not the one that was read: what is held
		// of its collection is what the write was refused for.
		if err == nil && (resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusConflict || resp.StatusCode == http.StatusPreconditionFailed) {
