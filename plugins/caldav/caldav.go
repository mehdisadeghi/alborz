...
func (p *plugin) clientWithCalendars(ctx context.Context, session *alborz.Session) (*caldav.Client, []CalendarInfo, error) {
	...
	infos, err := p.discoverCalendars(ctx, session, c)
	if err != nil {
		return nil, nil, err
	}
	return c, infos, nil
...
// discoverCalendars is the account's calendar list, empty when it has
...
func (p *plugin) discoverCalendars(ctx context.Context, session *alborz.Session, c *caldav.Client) ([]CalendarInfo, error) {
	davBase, _ := p.dav.URL(session)
	return p.calendars.Get(session.Username(), func() ([]CalendarInfo, error) {
